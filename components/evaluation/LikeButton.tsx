...
export function LikeButton({ evaluationId, initialIsLiked }: Props) {//いいねボタンのコンポーネントおよびプロパティに関する関数
  ...
  const [isLiked, setIsLiked] = useState(initialIsLiked);//いいね状態を画面に反映する
  ...
  return (//いいねボタンの見た目と動きを定義する
    <button
      ...
      onClick={() => {//クリックされた時の動きを定義する
        // ★ 即時UI更新
        setIsLiked(v => !v);//見た目を反転

        startTransition(() => toggleLike(evaluationId, pathname));////DBにいいね追加/削除されrevalidatePathが実行される
      }}
