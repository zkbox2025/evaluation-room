//⚪︎今後追加検討 ⚫︎情報
⚫︎DBのschema.prismaに書かれてあるlikeのevaluationIdは、microCMS の evaluation のコンテンツID。favoriteに書かれてあるpersonSlugは、microCMSのpersonのslug。
...
⭐️AI実装する際の注意（callLLMReviewで呼ぶ際のこと）
本番では必ず「実LLM実装」しか使えないようにする（環境変数で強制）
⚪︎RunAiReviewResultにLLM_ERROR や SNAPSHOT_ERROR を追加する。それに伴いrunAiReview.tsの返却やRunAiReviewButtonのUI分岐などを修正。callLLMReview 側で専用エラーを作るといい（LlmCallErrorなど）