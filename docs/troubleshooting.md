...
### [2026-03-10] [localhostでDB接続エラーが発生しサイトが閲覧できない]
...
**症状**
- 何が起きたか：サイト（localhost:3000/reviews）にアクセスすると Prisma のエラー(deviceIdを使ってviewerオブジェクト（viewerID入り）をデータベース（supabase）から探すことができない→cookieが取得できません)が発生し、ページが表示されない。
- 期待していた動作：データベースから情報を取得し、レビューページが表示されること

**再現手順**
1.Docker Desktop が停止した状態で npm run dev を実行。
2.ブラウザで localhost:3000/reviewsにアクセス。
3.サーバーログに PrismaClientKnownRequestError および ECONNREFUSED が出力される。

**エラーメッセージ / ログ**
- Error: P1001（データベースの接続エラー）: Can't reach database server at localhost:5432
- code: 'ECONNREFUSED'
- Cookieが無効の可能性があります。Cookieを有効にして再読み込みしてください。

**切り分けメモ（どこが怪しいか）**
- Prismaのコードミスではなく、インフラ（DBサーバー）との疎通が取れていない。
- docker ps（dockerの起動状態確認） が失敗することから、Docker自体の停止が疑われる。

**原因（Root Cause）**
- ローカルのDocker Desktopが起動しておらず、コンテナ上のPostgreSQLデータベースがオフラインになっていたため

**結論**
- データベースサーバーへの接続失敗により、アプリケーションがデータを取得できずクラッシュした。
- DBが死んでページ生成が失敗したせいで、Middlewareが作ったCookie（中身はdeviceID）でviewerIDの絞り込みができなかったため「Cookieが無効の可能性があります。Cookieを有効にして再読み込みしてください」と言う表示が「お気に入り一覧ページ」「いいね一覧ページ」「格レビューページ」に出た。


**解決策（Fix）**
- Docker Desktop アプリを起動（Finderのアプリケーションの中のDocker Desktopをクリック）。
- ターミナルで docker ps を実行し、DBコンテナが稼働していることを確認。
- localhost:3000/reviewsにアクセス

**確認（動作検証）**
- ブラウザでページをリロードし、正常にDBからデータが取得・表示されることを確認。

**よくある落とし穴**
- PC再起動後やスリープ復帰後に Docker Desktop が自動起動していない。
- ターミナル上のエラーだけを見て、コード（Prismaのクエリなど）を書き換えてしまう。


**再発防止（Prevention）**
- 開発開始時に docker ps で環境の健全性を確認する癖をつける。
- Docker Desktop の設定で「Start Docker Desktop when you log in」を有効にする
