...
ユーザーがAIを使って評価の部屋の言葉に対して、どんな背景があったの？とか認めた核心部分は何？とか時代を経て再度評価した二人の関係性の物語化とかもしも〇〇が✖️✖️したらとかそういう遊び方がある。その質問と回答の記録をDBに保存する。
2026/02/25
...
2026/03/16
⚫︎ミドルウェア（Middleware）とは「リクエストが目的地に届く前に、割り込んで特定の処理を行う関数」。レストランで言うと入り口の店員さん（予約の確認、人数のカウント、アルコール消毒のお願いなどを行う）。以下が主な役割
・認証・認可のチェック: 「ログインしている人だけ通す、していない人はログイン画面へ飛ばす」といった交通整理を行う
・リダイレクト・書き換え: 特定のURLへのアクセスを別のページへ転送したり、内部的にURLを書き換えたりする
・クッキー・ヘッダーの操作: ユーザーの言語設定（日本語か英語かなど）を読み取って適切なページを表示したり、セキュリティ用の情報をヘッダーに追加したりする
・ログの記録: 「いつ、誰が、どのページにアクセスしたか」という記録を自動的に取る