...
// ④ category表示名からURL用slug（日本語）を作成（簡易）
// microCMS側でカテゴリslugを持てるならそれを優先するのが理想（今は名前しか入力してないから、この関数でなんとかしとくよ。ゆくゆくはmicroCMSにcategories API（カテゴリーとslugを入力できる） を作ってpersonAPIのカテゴリーで参照する形式にしたい。
export function slugifyCategoryName(categoryName: string): string {//カテゴリー名からURL用のスラッグ(日本語)に形式を変換する関数
...
/**
 * ⑤ contentHtml の最大長（表示用の安全策）※文字切り特化型
 * - 長文でUIが崩れるのを防ぐ
