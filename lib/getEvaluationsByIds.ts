//microCMSからのEvaluationCMS（評価データ）を取得し呼び出しもとが使いやすいように変換して返す関数
...
export async function getEvaluationsByIds(ids: string[]): Promise<Evaluation[]> {//欲しいデータのIDをEvaluation配列で受け取る。
  ...
  const byId = new Map(data.contents.map((e) => [e.id, e]));//一度EvaluationIDをキーにした辞書（オブジェクト）を作る

  const ordered = ids.map((id) => byId.get(id)).filter(Boolean) as EvaluationCMS[];//最初に受け取った ids の順番（呼び出しもとからの指示書に書いてある順番）で辞書からデータを取り出すことで呼び出し側が期待した順番に整える
